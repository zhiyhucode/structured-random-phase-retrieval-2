...
from src.sr import StructuredRandom, generate_diagonal
...
class CompressedSensing(LinearPhysics):
    ...
    .. note::
        ...
        and :math:`m` due to the `Marchenko-Pastur law
        ...
        **kwargs,
        ...
        if self.fast:
            ...
            if mode == "gaussian":
                ...
                self._A /= np.sqrt(m)  # in-place division
                for _ in range(product - 1):
                    gaussian = torch.randn(
                        (m, m), device=device, dtype=dtype
                    ) / np.sqrt(m)
                    self._A = gaussian @ self._A
                    del gaussian  # delete gaussian matrix to save memory
            else:
                raise ValueError(
                    f"Unknown mode {mode}. Supported modes are 'gaussian', 'unitary', 'circulant' and 'product'."
                )
    ...
    def A_dagger(self, y: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if self.fast:
            ...
            if not hasattr(self, "_A_dagger"):
                if self.verbose:
                    print(
                        "Pseudo-inverse isn't defined, computing it. This can be slow if the matrix is large."
                    )
